...
$\lambda_i \mid r_i \sim \text{InvGamma}((\nu+1)/2, (\nu+r_i^2)/2)$,
...
At iteration~0, $\lambda_i$ are drawn from the prior $\text{InvGamma}(\nu/2,
\nu/2)$; subsequent iterations use the posterior conditional on the current
ensemble residuals, targeting down-weighting at actual outliers.
...
\begin{align}
\lambda &\sim \text{InvGamma}\!\left(\frac{\nu}{2}, \frac{\nu}{2}\right), \\
...
$Y \sim t_\nu(\mu, \sigma)$.
The parametrisation is chosen so that the underlying Gamma
random variable (before inversion) has mean~1.
The mean of the resulting inverse Gamma is
$\mathbb{E}[\lambda] = \nu/(\nu-2)$ for $\nu > 2$, meaning that for the
$\nu \approx 4.3$ observed in our application, typical observations
experience effective variance $\sigma^2/1.87 < \sigma^2$ and are
perturbed \emph{less} than under the Gaussian model.
The additional probability mass is concentrated in the tails, where
$\lambda \ll 1$ inflates variance for genuine outliers.
Outlier observations receive $\lambda_i \ll 1$, inflating their effective
variance to $\sigma^2 / \lambda_i$ and reducing their influence on
parameter estimates.
...
\varepsilon_i^{(j)},
\qquad \lambda_i \mid r_i \sim \text{InvGamma}\!\left(\frac{\nu+1}{2},\; \frac{\nu + r_i^2}{2}\right),
...
This confirms the intended separation of responsibilities: $\phi$
captures the baseline noise level (typical observation uncertainty),
while the $\lambda_i$ weights handle the rare large residuals.
...
Drawing $\lambda_i$ from their residual-conditional posterior for
$N_d = 11{,}323$ observations takes approximately 2~milliseconds (identical
to the prior draw).
