...
framework: the implementation reduces to drawing $\lambda_i$ from their
...
$Y \sim t_\nu(\mu, \sigma)$.
The parametrisation $\text{InvGamma}(\nu/2, \nu/2)$ ensures
$\mathbb{E}[\lambda] = 1$ for $\nu > 2$, so typical observations
experience effective variance $\sigma^2$---identical to the Gaussian
model.
