#!/usr/bin/env python3
"""Regenerate figures with v3 (proper Bayesian Cd) Student-t results."""
...
nu_vals = [8.0, 6.9, 6.1, 5.5, 5.1, 4.9, 4.7, 4.5, 4.5, 4.4, 4.4, 4.3]
...
phi_s = [0.7441, 0.7237, 0.6455, 0.5856, 0.5651, 0.5553, 0.5496, 0.5462, 0.5442, 0.5437, 0.5435, 0.5435]

fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 4.5))
ax1.plot(iters, nu_vals, "o-", color=C_STUDT, lw=2, markersize=6)
ax1.axhline(30, color="gray", ls="--", lw=1, alpha=0.5, label="Gaussian limit")
ax1.set_xlabel("Iteration"); ax1.set_ylabel("ν")
...
ax2.set_xlabel("Iteration"); ax2.set_ylabel("φ (mean noise std)")
ax2.set_title("(b) φ convergence", fontsize=11)
ax2.legend(fontsize=8); ax2.grid(True, alpha=0.3)

fig.suptitle("Student-t ARIES: ν and φ Convergence", fontsize=12, fontweight="bold", y=1.01)
fig.tight_layout()
...
print("Figure 2")
n_params = len(pn); n_cols = 5; n_rows = int(np.ceil(n_params / n_cols))
...
fig.suptitle("Posterior Marginals: Gaussian vs Student-t ARIES", fontsize=13, fontweight="bold", y=1.01)
fig.tight_layout()
...
for label, pp, nu_val in [("Gaussian", gp, None), ("Student-t", sp, 4.3)]:
    all_raw = np.array(Parallel(n_jobs=4, verbose=0, backend="threading")(delayed(run_model)(pp[j]) for j in range(len(pp))))
    all_t = np.clip(all_raw, 0, None) ** LAM
    pt_t = all_t[:, train_mask]
    Yt_t = np.clip(Y_train, 0, None) ** LAM
    ns = np.std(Yt_t - pt_t.mean(axis=0))
    rng = np.random.default_rng(42); nppc = min(500, len(pp))
    idx = rng.choice(len(pp), size=nppc, replace=False)
    noise_tr = rng.standard_t(df=nu_val, size=(nppc, pt_t.shape[1])) * ns if nu_val else rng.normal(0, ns, (nppc, pt_t.shape[1]))
    ppc_tr = (np.clip(np.clip(pt_t[idx], 0, None) + noise_tr, 0, None)) ** (1.0/LAM)
    if label == "Gaussian": g_ppc_tr = ppc_tr
    else: s_ppc_tr = ppc_tr

WINDOW = 365 * 3; d = all_obs_dates[:WINDOW]
fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(16, 8), sharey=True)
for ax, ppc, color, title in [(ax1, g_ppc_tr, C_GAUSS, "Gaussian"), (ax2, s_ppc_tr, C_STUDT, "Student-t (ν = 4.3)")]:
    ...
    ax.plot(d, ppc.mean(0)[:WINDOW], color=color, lw=1.2, label="PPC mean")
...
bars2 = ax.bar(x + w/2, [s_pf[p] for p in pcts], w, color=C_STUDT, alpha=0.8, label="Student-t (ν = 4.3)")
