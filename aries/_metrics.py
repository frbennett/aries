"""
Prediction Interval Coverage Probability (PICP) metrics for ensemble forecasts.
"""
...
def PICP(posterior, data, CI):
    ...
    for i in range(len(observed)):
        if lower[i] <= observed.value[i] <= upper[i]:
            included += 1
...
def PICP2(posterior, data, CI):
    """Compute PICP using CDF overlap method."""
    ...
    data_set["overlap"] = (
        norm.cdf(
            data_set["upper_quant"],
            loc=data_set["value"],
            scale=data_set["noise"],
        )
        - norm.cdf(
            data_set["lower_quant"],
            loc=data_set["value"],
            scale=data_set["noise"],
        )
    )
